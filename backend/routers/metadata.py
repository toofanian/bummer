...
def get_all_metadata(db=Depends(get_db), sp: spotipy.Spotify = Depends(get_spotify)):
...
def set_tier(
    ...
    result = (
        ...
        .upsert({"spotify_id": spotify_id, "tier": body.tier})
        .execute()
    )
...
def clear_tier(
    spotify_id: str, db=Depends(get_db), sp: spotipy.Spotify = Depends(get_spotify)
):
    result = (
        db.table("album_metadata")
        .upsert({"spotify_id": spotify_id, "tier": None})
        .execute()
    )
    return result.data[0]
...
def list_collections(db=Depends(get_db), sp: spotipy.Spotify = Depends(get_spotify)):
...
def create_collection(
    body: CollectionBody, db=Depends(get_db), sp: spotipy.Spotify = Depends(get_spotify)
):
...
def delete_collection(
    collection_id: str, db=Depends(get_db), sp: spotipy.Spotify = Depends(get_spotify)
):
...
def add_album_to_collection(
    ...
    result = (
        ...
        .insert({"collection_id": collection_id, "spotify_id": body.spotify_id})
        .execute()
    )
    return result.data[0]
...
def get_collection_albums(
    ...
    if not library_module._is_cache_fresh():
        ...
        library_module._cache["albums"] = [
            library_module._normalize_album(item) for item in all_items
        ]
